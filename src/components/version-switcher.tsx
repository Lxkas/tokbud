...
export function VersionSwitcher({ versions, defaultVersion }: { versions: string[]; defaultVersion: string }) {
	const [selectedBranch, setSelectedVersion] = React.useState(defaultVersion);
					...
					<DropdownMenuTrigger asChild>
						...
						>
							...
							<div className="flex flex-col gap-0.5 leading-none">
								...
								<span className="">{selectedBranch} Branch</span>
							</div>
					...
					<DropdownMenuContent className="w-[--radix-dropdown-menu-trigger-width]" align="start">
						{versions.map((version) => (
							<DropdownMenuItem key={version} onSelect={() => setSelectedVersion(version)}>
								{version} {version === selectedBranch && <Check className="ml-auto" />}
							</DropdownMenuItem>
						))}
					</DropdownMenuContent>
				</DropdownMenu>
